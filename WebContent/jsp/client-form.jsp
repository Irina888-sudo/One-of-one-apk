...
<%
    ...
    String messageRetour  = "";     // message affiché après soumission du formulaire

    if (modeModification) {
        // Récupérer le client existant pour pré-remplir les champs
    ...
    // ── TRAITEMENT DU FORMULAIRE SOUMIS (méthode POST) ───────────────────
    if ("POST".equals(request.getMethod())) {
        ...
        if (nom == null || nom.trim().isEmpty()) {
            ...
            if (modeModification) {
                ...
                messageRetour = ok ? "✅ Client modifié avec succès." : "❌ Erreur lors de la modification.";
            } else {
                ...
                messageRetour = ok ? "✅ Client ajouté avec succès." : "❌ Erreur lors de l'ajout.";
            }

            // Si succès → rediriger vers la liste après 1 seconde (via meta refresh)
            if (ok) {
...
<div class="main-content">
    ...
    <%-- Message d'erreur éventuel (si la validation a échoué) --%>
